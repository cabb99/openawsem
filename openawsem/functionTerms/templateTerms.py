...
def fragment_memory_term(oa, k_fm=0.04184, frag_file_list_file="./frag.mem", npy_frag_table="./frag_table.npy",
    ...
    from openawsem.memory import FragmentMemory, MemoryWells
    ...
    if isinstance(fragment_memory, FragmentMemory):
        ...
        memory = MemoryWells.from_frags_mem(frag_file_list_file, min_seq_sep=min_seq_sep,
                                            max_seq_sep=max_seq_sep, well_width=fm_well_width)
    return memory.to_openmm_force(oa, k_fm=k_fm, caOnly=caOnly, forceGroup=forceGroup,
                                  rmin=frag_table_rmin, rmax=frag_table_rmax, dr=frag_table_dr,
                                  cache=npy_frag_table, use_cache=UseSavedFragTable, debug=debug)
